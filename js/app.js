(function () {
    ...
    function drawStates(geojson, byTypeSort, color, facilityData) {
        ...
        // loop over each industry type
        byTypeSort.forEach(d => {
            // reset the range array
            range = []
            // loop over each state
            // console.log(geojson.features); // returns each individual state's info
            geojson.features.forEach(g => {
        ...
        barChart.selectAll('rect')
            // listen for a mouseover event on each rectangle
            .on('mouseover', (event, d) => {
                ...
                const type = d[0] // set default to all industries
    ...
    function drawPoints(geojson, facilityData, byTypeSort, svg) {
        ...
        // create a new projection our path generator for our map
        const projection = d3.geoAlbersUsa()
            .fitSize([width, height], geojson);
        const path = d3.geoPath()
            .projection(projection);
        ...
        // console.log(radius(25)); // output is 1.03
        // define color generator
        const color = d3.scaleOrdinal(d3.schemeCategory10)
            .domain(byTypeSort.map(d => d[0]));


        console.log(facilityData[0]["Facility_Name"]);
        const facilities = svg.append('g') // append new g element
            ...
            .attr('r', d => {
                // console.log(d); // access the data here
        ...
        let tooltipContent =
            `
           <b>Plant Type: ${d["Industry_Type"]}<br>
            Emissions by metric ton: ${d["Total"]}</b><br>
            ${d["Facility_Name"]}<br>
            ${d["City"]}, ${d["State"]}<br>
            `
